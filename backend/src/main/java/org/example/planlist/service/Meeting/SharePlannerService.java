...
import org.example.planlist.dto.MeetingDTO.response.FreeTimeIntervalDTO;
import org.example.planlist.dto.MeetingDTO.response.SharedPlannerResponseDTO;
...
public class SharePlannerService {
    ...
    public org.example.planlist.dto.MeetingDTO.response.SharedPlannerResponseDTO getSharedPlanner(Long plannerId) {
        ...
        MeetingSession meetingSession = meetingSessionRepository.findById(plannerId).orElseThrow();

        LocalDate startDate = meetingSession.getStartWeekDay();
        LocalDate endDate = meetingSession.getEndWeekDay();
        ...
        if (userIds.isEmpty()) {
            return new org.example.planlist.dto.MeetingDTO.response.SharedPlannerResponseDTO(
                    startDate + " ~ " + endDate,
...
//                    Collections.emptyList()
        ...
        List<org.example.planlist.dto.MeetingDTO.response.FreeTimeIntervalDTO> allList = new ArrayList<>();
...
//                notOneList
    ...
    private List<org.example.planlist.dto.MeetingDTO.response.FreeTimeIntervalDTO> intervalsToDTO(LocalDate date, List<Interval> intervals) {
        if (intervals.isEmpty()) return Collections.emptyList();

        List<org.example.planlist.dto.MeetingDTO.response.FreeTimeIntervalDTO> dtos = new ArrayList<>();
        for (Interval interval : intervals) {
            // 00:00 ~ 23:59:59 인 경우 allDay 처리
            if (interval.getStart().equals(LocalTime.MIN) && interval.getEnd().equals(LocalTime.MAX)) {
                dtos.add(org.example.planlist.dto.MeetingDTO.response.FreeTimeIntervalDTO.ofAllDay(date));
            } else {
    ...
    @Transactional
    public MeetingSession updateSelectTime(Long plannerId, SelectTimeRequestDTO dto) {
        // 기존 세션 찾기
        MeetingSession session = meetingSessionRepository.findById(plannerId)
