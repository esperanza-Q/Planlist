package org.example.planlist.service.MeetingSharePlanner;
...
public class MeetingSharePlannerService {
    ...
    public MeetingSharedPlannerResponseDTO getSharedPlanner(Long plannerId) {
        ...
        if (userIds.isEmpty()) {
            return new MeetingSharedPlannerResponseDTO(
                    startDate + " ~ " + endDate,
...
//                notOneList
    ...
    public PlannerSession updateSelectTime(Long plannerId, SelectTimeRequestDTO dto) {
        ...
        List<MeetingSession> allSessions = meetingSessionRepository.findByProject(project);

        LocalDate minDate = allSessions.stream()
                .filter(MeetingSession::getIsFinalized)
                .map(MeetingSession::getDate)
