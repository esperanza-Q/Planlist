package org.example.planlist.service.StandardSharePlanner;
...
public class StandardSharePlannerService {
    ...
    public StandardSharedPlannerResponseDTO getSharedPlanner(Long plannerId) {
        ...
        StandardSession standardSession = standardSessionRepository.findById(plannerId).orElseThrow();

        LocalDate startDate = standardSession.getStartWeekDay();
        LocalDate endDate = standardSession.getEndWeekDay();
        ...
        if (userIds.isEmpty()) {
            return new StandardSharedPlannerResponseDTO(
                    startDate + " ~ " + endDate,
...
//                notOneList
    ...
    public PlannerSession updateSelectTime(Long plannerId, SelectTimeRequestDTO dto) {
        // 1. 기존 세션 찾기
        StandardSession session = standardSessionRepository.findById(plannerId)
        ...
        List<StandardSession> allSessions = standardSessionRepository.findByProject(project);

        LocalDate minDate = allSessions.stream()
                .filter(StandardSession::getIsFinalized)
                .map(StandardSession::getDate)
        ...
        // 6. 세션 저장 후 반환
        return standardSessionRepository.save(session);
    }
}
